...
import socket
#module
from .captcha import CAPTCHA_IDS
...
class WebPage:
    ...
    def get_captcha_text(self, captcha_id: int, xpath: str, xpath2: str='') -> str:
        '''Returns the text for a captcha image at a given XPath'''
        with CAPTCHA_IDS[str(captcha_id)](self.driver) as cap:
            if xpath2:
                return cap.solve(xpath, xpath2)
            return cap.solve(xpath)

    def get_captcha_in_iframe(self, captcha_id: int, xpath: str, framexpath: str) -> str:
        with CAPTCHA_IDS[str(captcha_id)](self.driver) as cap:
            return cap.solve(xpath, framexpath)

    def execute_js(self, js_code: str) -> bool:
        '''Returns the results of given executed JavaScript code'''
        try:
            return self.driver.execute_script(js_code)
        except:
            logging.warning('WP - JS execution failed')
            return False
            
    # def js_pull_data(self, javascript: str) -> str:
    #     response = self.driver.execute_script(javascript)
    #     return response
    
    def get_json(self, js_code: str) -> dict:
        '''Returns the parsed data from javascript code into a dict'''
        try:
            ret_val = None
            response = self.driver.execute_script(js_code)
            if type(response) == str and '{' in response:
                ret_val = json.loads(json.dumps(response))
            else:
                raise Exception("Return value isn't valid")
            return ret_val
        except:
            logging.warning('WP - JS execution failed')
            return False

    def js_fill_input(self,xpath: str, text: str) -> bool:
        try:
            ...
            if element:
                ...
                return true;""".replace('[text]',text),element)
            return response
        except Exception as e:
            print(e)
            return True

    def get_link_text(self,xpath: str='') -> str:
        try:
            return self.get_text(xpath)
        except Exception as e:
            return ''

    def download_pdf_document(self,document_link: str,doc_download_new_window: bool=False) -> bool:
        ''' Return True if document successfully downloded else False '''
        try:
            self.driver.get(document_link)
            if doc_download_new_window:
                self.back()
            return True
        except:
            return False

    def download_pdf_document_by_xpath(self,xpath: str, wait_value) -> bool:
        try:
            try:
    ...
    def download_document_type9(self,xpath: str, link_xpath: str, wait_value, doc_download_new_window: bool=True) -> bool:
        try:
            if doc_download_new_window:
                self.click_element(xpath, new_window=True)
            else:
                self.click_element(xpath)
            sleep(wait_value//2)
            try:
                self.click_element(link_xpath)
            except Exception as e:
                print(e)
            sleep(wait_value//2)
            if doc_download_new_window:
